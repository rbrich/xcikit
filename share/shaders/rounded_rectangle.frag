...
layout(location = 1) in vec2 in_size;   // size in pixels
layout(location = 2) in vec2 in_thickness_radius;  // outline thickness, corner radius (in pixels)
...
void main() {
    ...
    if (all(greaterThan(abs(in_coord * in_size), is))) {
        ...
        if (attr.antialiasing > 0 || attr.softness > 0) {
            ...
            out_color = mix(color.fill, color.outline, smoothstep(r-f, r+f, lri));
            out_color = mix(out_color, vec4(0,0,0,0), smoothstep(r-f, r+f, lro));
        } else {
            out_color = mix(color.fill, color.outline, step(r, lri));
        }
    } else {
        ...
        if (attr.antialiasing > 0 || attr.softness > 0) {
            ...
            out_color = mix(color.fill, color.outline, smoothstep(1-f, 1+f, mri));
            out_color = mix(out_color, vec4(0,0,0,0), smoothstep(1-f, 1+f, mro));
        } else {
            out_color = mix(color.fill, color.outline, step(1, mri));
        }
    }
}
