...
pip3 install conan
# Libs
brew install pegtl libzip freetype sdl3 doxygen catch2 google-benchmark hyperscan
----
- macOS (MacPorts):
+
----
...
pip3 install conan
# Libs
port install vulkan-* MoltenVK libsdl3 harfbuzz-devel hyperscan libfmt catch2
----
...
Adjust `CONAN_INSTALL_ARGS` as needed. The deps with `system_x=True` won't be installed via conan, so the have to be present in the system.
