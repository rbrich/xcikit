...
3. We need these commands to work:
    ...
    * `glslc` (e.g. from https://vulkan.lunarg.com/sdk/home#windows[Vulkan SDK])

4. Beginning from the compiler:
...
5. Install Python and Conan:

    * Try `python`, if it brings up Microsoft Store with Python app, click "Get"
    * Install Conan either with `pip install conan` (requires adjusting PATH) or go to https://conan.io/downloads and choose Download x86_64 Installer. Install it, restart terminal to apply new PATH.
