...
pip3 install 'conan<2.0'
# Libs
port install vulkan-* MoltenVK glfw-devel harfbuzz-devel hyperscan libfmt catch2
----
...
5. Install conan: `pip install 'conan<2.0'` (Python should also work via VS developer tools)
...
Open the project, let the IDE load the top-level `CMakeLists.txt`.
Add the following cmake parameter, so it also installs Conan dependencies:

----
-DCONAN_INSTALL=ON
----

You still need `conan` command working.
