...
Any Unix-like OS with C++20 compliant compiler should work. There is no direct Windows support, but it's possible that the project will compile with some layer of Unix compatibility, e.g. WSL. Some of the libraries already compile natively, see <<porting-to-windows,Porting to Windows>>.
...
Use Conan profile with `os=Emscripten`, for example:
...
[build_requires]
[env]
# copy from CMake command line (when called with emcmake)
CONAN_CMAKE_TOOLCHAIN_FILE=.../cmake/Modules/Platform/Emscripten.cmake
CONAN_CMAKE_CROSSCOMPILING_EMULATOR=/usr/bin/node
----

See: https://docs.conan.io/en/latest/integrations/cross_platform/emscripten.html[]

Build scripts have basic support for Emscripten target:
