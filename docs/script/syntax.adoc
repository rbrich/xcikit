...
add2 = fun<T> a:T, b:T -> T {a + b}  // same as above, but with explicit type variable
...
[source,fire]
----
a:Int, b:Int -> Int                 // with parameter names (only after `fun` keyword)
Int, Int -> Int                     // without parameter names
(Int, Int) -> Int                   // same, the parentheses are optional
...
| 13   |  dot function call |  <arg> . <callable>
| (14) |  unary ops         |  -  +  !  ~
| (15) |  cast              |  <val> : Type
|===
...
with priority lower than dot-call, so in a combined expression dot-call takes precedence.
