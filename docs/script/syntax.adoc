...
[source,fire]
----
with out = (open "file.txt" "w") {
    // output stream is now redirected to a file
    write "this goes to file.txt"
}

with err = (tee (open "file.txt" "w") cerr) {
    log "this goes to cerr stream and also to the file"
}
----
...
The effects may be used for optimizations - a pure function can be automatically memoized,
for example.
...
is flagged with the `out` effect, the calling function is not flagged and can be
still considered pure and optimized accordingly.
...
In an imaginative C transpiler, the effects would be implemented as global
or thread-local pointers to the current streams and the currently thrown
exception.
