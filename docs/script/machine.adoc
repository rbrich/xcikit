...
are arguments. Some instructions use variable-length arguments, so
the instruction code is not enough for knowing how many following bytes
...
* `LOGICAL_OR` - Pulls two Bools from stack and pushes a Bool result
of `||` operation back.
* `SUBSCRIPT` - Has a type T as an instruction argument. Pulls a list of T
and a Int index from stack. Pushes result of `!` operation (of type T) back.

NOTE: All instructions are documented in `Code.h` and `Machine.cpp`.

=== Encoding of a type

Types are stored in modules. Any type from current module, builtin module
or an imported module can be referenced by an instruction.
The information that needs to be encoded in the instruction arguments
is which module contains the type and index of the type in the module's
type table.

Builtin types and types from current module are expected to be referenced
more frequently, so they have special encoding. Other (imported) modules
have a common varint encoding.

Two most-significant two bits of first byte decides which kind of encoding is used:

- `00 <idx:6>` - current module (first 64 types)
- `01 <idx:6>` - builtin types (first 64 types)
- `10 <mod:3> <idx:3>` - imported module (first 8 imported modules, first 8 types from each one)
- `11 <mod:Varint> <idx:Varint>` - any module, any type index (varint encoding)

Note that in the last kind of encoding, the varint of module index begins
with only 6 bits. The most-significant bit of those six is still used as the
continuation bit. Up to 5 bits of module index can fit this first byte,
resulting in 32 first imported modules still having single byte representation.
...
==== Shortened first byte

Spare bits from previous byte (which already contain some other information)
