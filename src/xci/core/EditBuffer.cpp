...
bool EditBuffer::delete_left()
{
    if (m_cursor == 0)
        return false;
    auto pos = m_content.crbegin() + ssize_t(m_content.size()) - ssize_t(m_cursor);
    auto prev = m_content.crbegin() + ssize_t(m_content.size()) - utf8_prev(pos);
...
bool EditBuffer::delete_right()
{
    if (m_cursor >= m_content.size())
        return false;
    auto next = utf8_next(m_content.cbegin() + ssize_t(m_cursor)) - m_content.cbegin();
...
bool EditBuffer::move_left()
{
    if (m_cursor == 0)
        return false;
    auto pos = m_content.crbegin() + ssize_t(m_content.size()) - ssize_t(m_cursor);
    m_cursor = m_content.crbegin() + ssize_t(m_content.size()) - utf8_prev(pos);
    return true;
}
...
bool EditBuffer::move_right()
{
    if (m_cursor >= m_content.size())
        return false;
    m_cursor = utf8_next(m_content.cbegin() + ssize_t(m_cursor)) - m_content.cbegin();
    return true;
}
...
static bool is_word_char(char32_t c)
{
...
{
    const auto rlast = m_content.crbegin() + ssize_t(m_content.size());
    const auto new_cursor = rlast - utf8_prev(rlast - ssize_t(m_cursor));
