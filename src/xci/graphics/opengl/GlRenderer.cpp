...
ShaderPtr GlRenderer::new_shader(Renderer::ShaderId shader_id)
{
    if (shader_id != Renderer::ShaderId::Custom) {
        ...
        if (!shader) {
            shader = ShaderPtr(new Shader(m_file_watch));
        }
        return shader;
    } else {
        assert(!"Custom shaders not implemented");
        return ShaderPtr();
    }
}
...
bool
GlRenderer::shader_load_from_file(ShaderPtr& shader,
                                  ...
                                  const std::string& fragment)
{
    ...
    if (!program) {
        ...
        shader->add_watches(vertex, fragment);
...
bool GlRenderer::shader_load_from_memory(ShaderPtr& shader,
...
void Shader::set_program(GLuint program)
{
...
void Shader::add_watches(const std::string& vertex, const std::string& fragment)
{
